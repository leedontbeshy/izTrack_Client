<template>
  <Card class="w-full max-w-md bg-dark-card border-dark-card">
    <CardHeader>
      <CardTitle class="text-off-white text-2xl font-bold">ĐĂNG NHẬP</CardTitle>
      <CardDescription class="text-off-white/60"> Đăng nhập vào tài khoản của bạn </CardDescription>
    </CardHeader>
    ...
    <CardContent>
      <form class="space-y-4" @submit.prevent="handleLogin">
        ...
        <!-- Email Input -->
        <div class="space-y-2">
          <label for="login-email" class="text-sm font-medium leading-none text-off-white">
            Email
          </label>
          <Input
            id="login-email"
        ...
        <!-- Password Input -->
        <div class="space-y-2">
          <label for="login-password" class="text-sm font-medium leading-none text-off-white">
            Mật khẩu
          </label>
          <Input
            id="login-password"
...
const handleGoogleSignIn = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      error.value = result.error || 'Đăng nhập bằng Google thất bại';
    }
  } finally {
    loading.value = false;
  }
